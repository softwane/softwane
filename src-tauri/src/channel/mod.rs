...
macro_rules! define_channels {
    (
        $(
            $type_var:ident(default_switch_on=$switch_on:literal) => $val_var:ident($data_type:ty, neutral=$val_neutral:literal, default_peak=$default_peak:literal)
        );* $(;)? // 允许末尾带分号
    ) => {
        ...
        impl std::ops::Mul<f32> for ChannelValue {
            type Output = Self;
            #[allow(clippy::unnecessary_cast)]  // Suppress unnecessary cast warning
            fn mul(self, other: f32) -> Self {
                match self {
                    $( 
                        // Convert to f32 for multiplication, then convert back to target type ($data_type)
                        // If there are non-primitive types, we need to change this implementation
                        Self::$val_var(a) => Self::$val_var((a as f32 * other) as $data_type), 
                    )*
                }
            }
        }
        ...
        pub struct PersistentChannelFuncParamsMatrix {
            ...
            progress_begin_time_ms: u64,
            peak_channel_value: ChannelValue,
        }

        impl ChannelFuncParamsMatrix {
            pub fn new(persistent_params_mat: &PersistentChannelFuncParamsMatrix) -> Self {
                let channel_type: ChannelType = persistent_params_mat.peak_channel_value.into();
                let progress_params = ChannelFuncParams {
                    curve_parameters: persistent_params_mat.progress_curve_parameters,
                    curve_begin_time_ms: persistent_params_mat.progress_begin_time_ms,
                    curve_begin_value: channel_type.neutral_value(),
                    target_value: persistent_params_mat.peak_channel_value,
                };
                ...
                let reverse_params = ChannelFuncParams {
                    ...
                    curve_begin_time_ms: 0,
                    curve_begin_value: persistent_params_mat.peak_channel_value,
            ...
            pub fn persist(&self) -> PersistentChannelFuncParamsMatrix {
                ...
                PersistentChannelFuncParamsMatrix {
                    ...
                    progress_begin_time_ms: progress_params.curve_begin_time_ms,
                    peak_channel_value: progress_params.target_value,
                }
            }
        }
        ...
        impl SensoryChannel {
            pub fn new(config: SensoryChannelConfigs) -> Self {
                let channel_type: ChannelType = config.persistent_params_mat.peak_channel_value.into();
                Self {
            ...
            pub fn handle_command(&mut self, command: &ChannelCommand) {
                match command {
                    ...
                    ChannelCommand::UpdatePeakChannelValue { peak_channel_value } => {
                        self.function_params_matrix[&PROGRESS_STATE].target_value = *peak_channel_value;
                        self.function_params_matrix[&SETTLING_STATE].target_value = *peak_channel_value;
                    }
                    ChannelCommand::UpdateProgressBeginTime { channel_type: _, progress_begin_time_ms } => {
                        self.function_params_matrix[&PROGRESS_STATE].curve_begin_time_ms = *progress_begin_time_ms;
                    }
            ...
            fn calculate_this_value(&self, state: &TimerState) -> ChannelValue {
                ...
                if elapsed_ms < func_params.curve_begin_time_ms {
                    return func_params.curve_begin_value;
                }
                if elapsed_ms > target_duration_ms {
                    return func_params.target_value;
                }
                let progress = (elapsed_ms - func_params.curve_begin_time_ms) as f32 / (target_duration_ms - func_params.curve_begin_time_ms) as f32;
        ...
        pub const DEFAULT_SENSORY_CHANNELS_CONFIGS: SensoryChannelsConfigs = [
            $( 
                SensoryChannelConfigs {
                    switch_on: $switch_on,
                    persistent_params_mat: PersistentChannelFuncParamsMatrix {
                        progress_curve_parameters: CurveParameters::NormalizedSigmoid { steepness: 10.0 },
                        settling_curve_parameters: CurveParameters::NormalizedSigmoid { steepness: 10.0 },
                        reverse_curve_parameters: CurveParameters::NormalizedSigmoid { steepness: 10.0 },
                        progress_begin_time_ms: 0,
                        peak_channel_value: ChannelValue::$val_var(<$data_type>::from($default_peak)),
                    },
                },
            )*
        ];
...
define_channels!(
    Saturation(default_switch_on=true)  => Saturation(f32, neutral=1.0, default_peak=0.2);
    Warmth(default_switch_on=true)      => WarmthKelvin(u16, neutral=6500_u16, default_peak=2500_u16);
    Brightness(default_switch_on=false) => Brightness(f32, neutral=1.0, default_peak=0.6);
);
...
mod curve_functions {
    ...
    fn sigmoid(x: f32, steepness: f32) -> f32 {
        1.0 / (1.0 + f32::exp(-steepness * (x - 0.5)))
    }
    pub(super) fn normalized_sigmoid(x: f32, steepness: f32) -> f32 {
